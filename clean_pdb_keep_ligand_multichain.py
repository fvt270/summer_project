...
fastaseq = ""
pdbfile = ""
def check_and_print_pdb( count, residue_buffer, residue_letter,chainid ):
  global fastaseq
  global pdbfile
  global CA_MODEL
  global NO_OCCUPANCY
...
for i in range(len(lines)):
  ...
  print("len(chainid):", len(chainid))
  if (chainid == line[21] or ignorechain):
      line_edit = line
      if line[0:3] in ['TER', "END"]:
        continue
      ...
      if (line_edit[0:4] == 'ATOM') or (line_edit[0:6] == 'HETATM' ):
        ...
        ##                    resnum = line_edit[23:26]
        if(line[0:6] == 'HETATM'):
          if (change_lig_id == True):
            line_edit = line_edit[:20]+' X'+line_edit[22:]
        ...
        if not resnum == oldresnum:
          if residue_buffer != []:  ## is there a residue in the buffer ? If so print it out, then reinitialize
            if not residue_invalid:
              ...
              if not check_and_print_pdb( count, residue_buffer, residue_letter, chainid ):
                ## if unsuccessful
