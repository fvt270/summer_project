    ...
    for files in os.listdir (run_dir):
        if files.startswith("mutfile"):
            ...
            if os.path.getsize(out_add) > 0:
                ...
                def get_vars(x):
                    print(f"get_vars beggining")
                    print(f"x = {x}")
                    res: list = x.split('_')[1:]
                    print(f"res = {res}\nLength of res: {len(res)}")
                    resis_list = []
                    mut_var_list = []
                    blacklist = []
                    for var_res in res:
                        print(f"var_res before split: {var_res}")
                        var_res = var_res.split(':')[0]       
                        print(f"var_res after split: {var_res}")                 
                        mut_var = var_res[-3:]
                        print(f"mut_var initial: {mut_var}")
                        try:
                            mut_var = d1t3[mut_var]
                        except:
                            print("Couldn't find amino acid mut_var:", mut_var)
                            resis_list.append(int(var_res[:-3])) # For ignoring later - still need to be in list
                            blacklist.append((str(var_res[:-3])))# For ignoring later
                            blacklist.append((int(var_res[:-3])))# For ignoring later
                            mut_var_list.append(mut_var) # For ignoring later
                            continue
                        #-
                        resi = int(var_res[:-3])
                        print(f"resi: {resi}")
                    ...
                    print(f"wt_var initial: {wt_var}")
                    wt_var = wt_var.split(':')
                    print(f"wt_var after split: {wt_var}")
                    
                    tmp_joined_list = []
                    for ind, resi in enumerate(resis_list):
                        if resi in blacklist: # Ignore invalid Amino Acids mutations
                            print("resi:", resi, "is blacklisted and will be added as wt;resi;wt")
                            wt = wt_var[ind][0]
                            mut = wt_var[ind][0]
                        else:
                            print(f"ind: {ind}, resi: {resi}")
                            wt = wt_var[ind][0]
                            mut = mut_var_list[ind]
                        #-
                        print("String that will be appended:")
                        print(f"{wt}{resi}{mut}")
                        tmp_joined_list.append(f"{wt}{resi}{mut}")
                    #-
