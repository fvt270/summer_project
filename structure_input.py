...
class structure:
    ...
    def __init__(self,chain_id,name,folder,prep_struc,run_struc,logger,input_dict,uniprot_accesion=''):
        ...
        #logger(chain_id,name,folder,prep_struc,run_struc,input_dict,uniprot_accesion)
        logger.info(f' {chain_id}, {name}, {folder}, {prep_struc}, {run_struc}, {logger}, {input_dict}, {uniprot_accesion}')
        print(chain_id,name,folder,prep_struc,run_struc,input_dict,uniprot_accesion)
        try:
            ...
            print(self.struc_dic)
            self.folder=folder
            self.logger=logger
            self.input_dict=input_dict
            self.fasta_seq_all=''
            print("Readying uniprot accesion")
            if uniprot_accesion != '':
                if os.path.isfile(uniprot_accesion):
                    print("File is uniprot accesion")
                    self.uniprot_seq = read_fasta(uniprot_accesion)
                else:
                    print("File is not uniprot accession")
    ...
    def clean_up_and_isolate(self, name='input',ligand=None):
        ...
        if  ligand == None:
            ...
            print(f"self.path_to_clean_pdb: {self.path_to_clean_pdb}")
            #remove all HETATM
            ...
            #Runs shell script
            shell_command = f'/lustre/hpc/sbinlab/software/anaconda2/bin/python2 {self.path_to_clean_pdb} {self.prep_struc} {self.run_struc}  --keepzeroocc'
            print("shell_command:", shell_command)
            self.logger.info(f'Running clean_pdb.py script {shell_command}')
        ...
        if ligand == True:
            if len(list(self.run_struc))>1:
                for chainn in [x for x in self.run_struc]:
                    print("chainn:", chainn)
                    self.path_to_clean_pdb = rosetta_paths.path_to_clean_keep_ligand_multichain ##MLS 
                    print("self.path_to_clean_pdb:", self.path_to_clean_pdb)
                    self.tmp_prep_struc = create_copy(self.prep_struc, self.folder.prepare_cleaning, name='withHETATM.pdb')
                    if chainn[0] == "/":
                        pass
                    else: # Absolute path
                        chainn = "/" + chainn
                    #Runs shell script
                    shell_command = f'/lustre/hpc/sbinlab/software/anaconda2/bin/python2 {self.path_to_clean_pdb} {self.tmp_prep_struc} {chainn}  --keepzeroocc'
                    print("shell_command:", shell_command)
                    self.logger.info(f'Running clean_pdb_keep_ligand_multichain.py script {shell_command}')
                    subprocess.call(shell_command, cwd=self.folder.prepare_cleaning, shell=True)
                    self.logger.info('end of output from clean_pdb_keep_ligand_multichain.py')
                
                path_to_cleaned_pdb = f'{self.tmp_prep_struc}{self.run_struc}.pdb'
                print(f"path_to_cleaned_pdb: {path_to_cleaned_pdb}")
                with open(path_to_cleaned_pdb, 'w') as fp:
                    for chainn in [x for x in self.run_struc]:
                        #path_to_cleaned_pdb_tmp = f'{self.tmp_prep_struc}_{self.run_struc}.pdb' ##MLS
                        path_to_cleaned_pdb_tmp = f'{self.tmp_prep_struc}{chainn}.pdb'
                        print(f"path_to_cleaned_pdb_tmp: {path_to_cleaned_pdb_tmp}")
                ...
                self.tmp_prep_struc = create_copy(self.prep_struc, self.folder.prepare_cleaning, name='withHETATM.pdb')
                #Runs shell script
                shell_command = f'/lustre/hpc/sbinlab/software/anaconda2/bin/python2 {self.path_to_clean_pdb} {self.tmp_prep_struc} {self.run_struc}  --keepzeroocc'
